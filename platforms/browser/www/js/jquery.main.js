...
function initTabsHeader() {
    jQuery(function() {
        jQuery("#demoTab1").easyResponsiveTabsHeader();
    });
}
function initTabsOnMap() {
    jQuery(function() {
        jQuery("#demoTab2").easyResponsiveTabsOnMap();
    });
}
...
function initLoadMore() {
    jQuery('.project-info').each(function() {
        jQuery("li").slice(0, 4).show(); // select the first ten
        jQuery("#loadMore").click(function(e){ // click event for load more
            e.preventDefault();
            jQuery("li:hidden").slice(0, 4).show(); // select next 10 hidden divs and show them
            if(jQuery("li:hidden").length == 0){ // check if any hidden divs still exist
...
(function ($) {
    $.fn.extend({
        easyResponsiveTabsHeader: function (options) {
...
(function ($) {
    $.fn.extend({
        easyResponsiveTabsOnMap: function (options) {
            //Set the default values, use comma to separate the settings, example:
            var defaults = {
                type: 'default', //default, vertical, accordion;
                width: 'auto',
